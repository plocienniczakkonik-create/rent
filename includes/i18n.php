...
class i18n
{
    ...
    public static function init()
    {
        ...
        // Set admin language from session first, then user profile
        if (isset($_SESSION['admin_language'])) {
            self::$current_admin_language = $_SESSION['admin_language'];
            ...
            self::$current_admin_language = $default_admin_language;
        }
    ...
    public static function renderLanguageSwitcher($context = 'admin', $current_page = '')
    {
        ...
        // Upewnij się, że mamy BASE_URL
        $base_url = defined('BASE_URL') ? BASE_URL : '';

        // Jeśli current_page jest puste, użyj current URL
        if (empty($current_page)) {
            $current_page = $_SERVER['REQUEST_URI'] ?? '';
        }

        $html = '<div class="language-switcher d-flex align-items-center">';

        foreach ($languages as $code => $lang) {
            $active = ($code === $current_lang) ? 'text-dark fw-bold' : 'text-secondary';

            // Buduj URL z zachowaniem obecnej strony
            $url = $base_url . '/index.php?set_language=' . $code . '&context=' . $context . '&return=' . urlencode($current_page);

            $html .= '<a href="' . htmlspecialchars($url) . '" class="text-decoration-none ' . $active . ' me-2" title="' . $lang['name'] . '">';
...
if (isset($_GET['set_language']) && isset($_GET['context'])) {
    ...
    if ($context === 'admin') {
        ...
        // Ustaw język dla wszystkiego - frontend i admin
