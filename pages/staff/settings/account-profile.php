...
if (!$user) {
    echo '<div class="alert alert-danger">' . __('user_not_found', 'admin', 'Nie znaleziono danych użytkownika.') . '</div>';
    return;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($last_name)) {
        $errors[] = __('last_name_required', 'admin', 'Nazwisko jest wymagane');
    }
    ...
    if (!empty($new_password)) {
        if (strlen($new_password) < 6) {
            $errors[] = __('password_min_length', 'admin', 'Hasło musi mieć co najmniej 6 znaków');
        }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h5 class="mb-1"><?= __('account_settings', 'admin', 'Ustawienia konta') ?></h5>
        <p class="text-muted mb-0"><?= __('manage_personal_data', 'admin', 'Zarządzaj swoimi danymi osobowymi i preferencjami') ?></p>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0"><i class="bi bi-person-circle"></i> <?= __('personal_information', 'admin', 'Informacje osobiste') ?></h6>
            </div>
            <div class="card-body">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0"><i class="bi bi-info-circle"></i> <?= __('account_info', 'admin', 'Informacje o koncie') ?></h6>
            </div>
            <div class="card-body">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-6">
                        <small class="text-muted"><?= __('status', 'admin', 'Status') ?>:</small>
                        <div>
                    ...
                    <div class="col-6">
                        <small class="text-muted"><?= __('created', 'admin', 'Utworzone') ?>:</small>
                        <div><?= date('d.m.Y', strtotime($user['created_at'])) ?></div>
                    </div>
                    <div class="col-6">
                        <small class="text-muted"><?= __('last_update', 'admin', 'Ostatnia aktualizacja') ?>:</small>
                        <div>
                            <?php if ($user['updated_at']): ?>
                                <?= date('d.m.Y H:i', strtotime($user['updated_at'])) ?>
                            <?php else: ?>
                                <?= __('never', 'admin', 'Nigdy') ?>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <?php if ($user['job_title']): ?>
                    <hr>
                    <div>
                        <small class="text-muted"><?= __('job_title', 'admin', 'Stanowisko') ?>:</small>
            ...
            <div class="card-body">
                <div class="d-grid gap-2">
                    <button class="btn btn-outline-primary btn-sm" disabled>
                        <i class="bi bi-clock-history"></i> <?= __('activity_history', 'admin', 'Historia aktywności') ?>
                    </button>
                    <button class="btn btn-outline-secondary btn-sm" disabled>
                        <i class="bi bi-phone"></i> <?= __('2fa_authentication', 'admin', 'Uwierzytelnianie 2FA') ?>
                    </button>
                    <button class="btn btn-outline-warning btn-sm" disabled>
                        <i class="bi bi-key"></i> <?= __('active_sessions', 'admin', 'Sesje aktywne') ?>
                    </button>
                </div>
                <p class="text-muted small mt-2 mb-0">
                    <?= __('security_features_coming', 'admin', 'Funkcje bezpieczeństwa będą dostępne w kolejnych wersjach.') ?>
                </p>
            </div>
        </div>
    </div>
