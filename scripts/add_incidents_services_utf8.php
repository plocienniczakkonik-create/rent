<?php
// Skrypt dodający dane incydentów i serwisów z prawidłowym kodowaniem UTF-8
...
echo "Dodawanie danych serwisów i incydentów z prawidłowym kodowaniem UTF-8...\n\n";
...
try {
    ...
    $stmt = $db->prepare("INSERT INTO vehicle_services (vehicle_id, service_date, odometer_km, workshop_name, invoice_no, cost_total, issues_found, actions_taken, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

    foreach ($services as $service) {
        $stmt->execute($service);
    }

    echo "✓ Dodano " . count($services) . " serwisów\n";
    ...
    $stmt = $db->prepare("INSERT INTO vehicle_incidents (vehicle_id, incident_date, driver_name, location, damage_desc, fault, police_called, repair_cost, insurance_claim_no, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    foreach ($incidents as $incident) {
        $stmt->execute($incident);
    }

    echo "✓ Dodano " . count($incidents) . " incydentów\n";
    echo "\n✅ ZAKOŃCZONO POMYŚLNIE!\n";
    echo "Wszystkie dane zostały dodane z prawidłowym kodowaniem UTF-8.\n";
} catch (Exception $e) {
